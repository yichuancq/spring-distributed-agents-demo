...
public class AgentController {
    ...
    public Flux<String> streamInvokeAgent(
            @PathVariable String agentName,
            @RequestBody AgentRequest request) {
        ...
        if (result.isPresent()) {
            return Flux.just(
                    "data: {\"agent\": \"" + agentName + "\", \"status\": \"processing\"}\n\n",
                    "data: {\"agent\": \"" + agentName + "\", \"result\": \"" + 
                            escapeJson(result.get()) + "\"}\n\n",
    ...
    public ResponseEntity<AgentListResponse> listLocalAgents() {
        ...
        List<AgentCard> localAgents = localAgentNames.stream()
                .map(name -> a2AAgentService.getAgentCard(name))
    ...
    public ResponseEntity<AgentResponse> registerAgent(
        ...
        return ResponseEntity.ok(AgentResponse.success(agentName, 
                "Agent 注册成功: " + agentName));
    }
